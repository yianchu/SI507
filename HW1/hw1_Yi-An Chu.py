...
% Student Name: Yi-An Chu

% Student Unique Name: yianchu

% Lab Section 00X: 004

% I worked with the following classmates: No
...
def make_move(player, board):
    ...
    print(player_name(player)+"'s move:")
    str = input()
    while True:
        try:
            move = int(str)
            #print(unoccupy)
            if unoccupy[move - 1] != 0:
                #unoccupy[move-1] = 0
                #print(move)
                break
            else:
...
while(moves_left > 0 and winner == 0):
    ...
    moves_left -= 1
if winner != 0:
    print('Game over! ' + player_name(pre_player) + ' wins!')
else:
